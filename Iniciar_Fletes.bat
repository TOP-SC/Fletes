...
"%PY%" "%~dp0scripts\kill_api_port.py"
timeout /t 2 /nobreak >nul

echo [2/4] Iniciando servidor API (segundo plano)...
start "Fletes-API" /min cmd /c "cd /d "%~dp0backend" && set PYTHONPATH=%~dp0backend && "%PY%" -m uvicorn app.main:app --host 127.0.0.1 --port 8000"

echo [3/4] Esperando API...
"%PY%" "%~dp0scripts\wait_api.py"
